...
/// Accumulate 'value' into histogram 'buckets', using a minimal number of
/// memory operations
inline __device__ uint32_t reduce(uint32_t active, uint32_t value,
                                  uint32_t *buckets) {
...
/// Atomically accumulate 'value' into histogram 'buckets', using a minimal
/// number of atomic operations
inline __device__ uint32_t reduce_atomic(uint32_t active, uint32_t value,
                                         uint32_t *buckets) {
  uint32_t peers = get_peers(active, value);
 ...
 */
KERNEL void mkperm_phase_1_tiny(const uint32_t *values, uint32_t *buckets,
                                uint32_t size, uint32_t size_per_block,
                                uint32_t bucket_count) {
  uint32_t *shared = SharedMemory<uint32_t>::get();

  uint32_t thread_id = threadIdx.x, thread_count = blockDim.x,
           block_start = blockIdx.x * size_per_block,
           block_end = block_start + size_per_block,
           warp_count = thread_count / warpSize, warp_id = thread_id / warpSize;
 ...
 */
KERNEL void mkperm_phase_1_small(const uint32_t *values, uint32_t *buckets,
                                 uint32_t size, uint32_t size_per_block,
                                 uint32_t bucket_count) {
  uint32_t *shared = SharedMemory<uint32_t>::get();
 ...
 */
KERNEL void mkperm_phase_1_large(const uint32_t *values, uint32_t *buckets_,
                                 uint32_t size, uint32_t size_per_block,
                                 uint32_t bucket_count) {
  uint32_t thread_id = threadIdx.x, thread_count = blockDim.x,
           block_start = blockIdx.x * size_per_block,
...
KERNEL void mkperm_phase_3(uint32_t *buckets, uint32_t bucket_count,
                           uint32_t bucket_count_rounded, uint32_t perm_size,
                           uint32_t *counter, uint4 *offsets) {
  uint32_t *shared = SharedMemory<uint32_t>::get();
...
/// Generate a sorting permutation based on offsets generated by \ref
/// mkperm_phase_1_tiny()
KERNEL void mkperm_phase_4_tiny(const uint32_t *values,
                                const uint32_t *buckets_, uint32_t *perm,
                                uint32_t size, uint32_t size_per_block,
                                uint32_t bucket_count) {
  uint32_t *shared = SharedMemory<uint32_t>::get();
...
KERNEL void transpose(const uint32_t *in, uint32_t *out, uint32_t i_rows,
                      uint32_t i_cols) {
  uint32_t *shared = SharedMemory<uint32_t>::get();
  ...
  uint32_t o_rows = i_cols, o_cols = i_rows,
