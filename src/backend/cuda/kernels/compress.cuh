...
KERNEL void compress_small(const uint8_t *in, uint32_t *out, uint32_t size,
                           uint32_t *count_out) {
...
KERNEL void compress_large(const uint8_t *in, uint32_t *out, uint64_t *scratch,
                           uint32_t *count_out) {
  uint32_t *shared = SharedMemory<uint32_t>::get();
  ...
  uint8_t values_8[17];
  *(uint4 *)values_8 =
      ((const uint4 *)in)[blockIdx.x * thread_count + threadIdx.x];
  values_8[16] = 0;
