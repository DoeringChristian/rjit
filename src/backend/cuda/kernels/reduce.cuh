...
template <typename Value> struct reduction_add {
  ...
  __device__ Value operator()(Value a, Value b) const { return a + b; }
};
...
template <typename Value> struct reduction_max {
  ...
  __device__ Value operator()(Value a, Value b) const { return max(a, b); }
};
...
template <typename Value> struct reduction_min {
  ...
  __device__ Value operator()(Value a, Value b) const { return min(a, b); }
};
