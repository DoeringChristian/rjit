...
void jit_scan_u32(const uint32_t *in, uint32_t *out, uint32_t size) {
  uint32_t items_per_thread = 16, thread_count = 128,
  ...
  scan_large_u32<<<block_count, thread_count, shared_size>>>(in, out,
                                                             block_sums + 32);
  cudaFree(block_sums);
}
...
int main(int argc, char **argv) {
  ...
  cuda_check(
      cub::DeviceScan::ExclusiveSum(temp, temp_size, ptr, ptr_out, size));
  ...
  cuda_check(
      cudaMemcpy(ptr_cpu, ptr_out, size * sizeof(uint32_t), cudaMemcpyDefault));
  cuda_check(cudaMemcpy(ptr_cpu_2, ptr_out_2, size * sizeof(uint32_t),
                        cudaMemcpyDefault));
