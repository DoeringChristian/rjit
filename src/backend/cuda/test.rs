...
fn tex_lookup_8() -> Result<()> {
    ...
    let data = ir.array(&[1.0f32; 800])?;

    let tex = data.to_texture(&[10, 10], 8)?;
...
fn tex_lookup_3D_8() -> Result<()> {
    ...
    let x = ir.array(&[0.5f32])?;
    let y = ir.array(&[0.5f32])?;
    let z = ir.array(&[0.5f32])?;

    let data = ir.array(&[1.0f32; 8000])?;

    let tex = data.to_texture(&[10, 10, 10], 8)?;

    let res = tex.tex_lookup(&[&x, &y, &z])?;
